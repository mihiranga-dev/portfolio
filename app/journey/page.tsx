    ...
    <div className="max-w-4xl mx-auto py-12 md:py-20 space-y-12">
      <div className="space-y-4">
        ...
        <p className="text-lg text-slate-500 dark:text-slate-400 max-w-2xl">
          Tracing my path from independent exploration to formal enterprise engineering and community leadership.
        </p>
      </div>
        ...
        {journey.map((item) => (
          ...
          >
            ...
            <div className="flex flex-col items-center md:items-start gap-4 md:w-48 shrink-0">
              <div className="p-4 rounded-2xl bg-primary/10 text-primary border border-primary/20 shadow-[0_0_20px_rgba(124,58,237,0.1)]">
              ...
              <div className="flex flex-col items-center md:items-start space-y-1">
                <span className="text-2xs font-bold uppercase flex items-center gap-1.5">
                  <Calendar className="w-3 h-3" /> {item.date}
                </span>
                <span className="text-[10\2px] text-slate-400 font-mono uppercase">
                  {item.category}
                </span>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {item.tags.map((tag) => (
                  <span 
                    key={tag} 
                    className="px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-md bg-slate-100 hover:scale-105 transition-transform dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-gray-700"
                  >
                    {tag}
                  </span>
                ))}
