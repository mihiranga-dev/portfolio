      ...
      <div className="space-y-6 mb-16">
        <div className="flex flex-wrap gap-2">
          {project.tech.map((t) => (
            <span
              key={t}
              className="px-3 py-1 text-[10px] font-bold uppercase tracking-widest bg-primary/10 text-primary rounded-md border border-primary/20"
            >
              {t}
            </span>
          ))}
        ...
        {project.isContributed && (
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20">
            ...
            <span>
              Team Collaboration •{" "}
              <strong>{project.role ?? "Contributor"}</strong>
            </span>
          </div>
        )}
        ...
        <div className="flex gap-4 pt-4">
          ...
          {project.liveDemo && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 px-6 py-3 border border-slate-200 dark:border-gray-800 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-gray-900 transition-colors"
            >
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 rounded-2xl bg-slate-50 dark:bg-gray-900/50 border border-slate-100 dark:border-gray-800 flex items-start gap-4">
            ...
            <p className="text-xs font-mono text-slate-500">
              &quot;Focusing on system optimization and clean data
              patterns.&quot;
            </p>
          </div>
        </div>
